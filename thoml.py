...
class MemoryBuffer:
    ...
    def sample(self, batch_size: int, pop: bool = False) -> List[
        Tuple[torch.Tensor, torch.IntTensor, torch.IntTensor, torch.FloatTensor, torch.Tensor, torch.BoolTensor]]:
...
class MetaNet():
    def __init__(self, state_dim: int, action_dim: int, hidden_dim: int = 64, layers: int = 3,
                 ...
                 gamma: float = 0.99, lossFn: Callable = nn.SmoothL1Loss, inner_lr: float = 0.0001,
                 heuristic_update_inner_lr: bool = False,
                 tau: float = 0.001, num_steps: int = 500, update_rate: int = 8, batch_size: int = 32,
        ...
        if (useMetaSampling and use_reptile) or (not useMetaSampling and not use_reptile):
            raise ValueError(
                "The use of reptile and meta-sampling is mutually exclusive. Please set only one to True/False.")
        ...
        if self.useMetaSampling:
            self.inner_target_net = QNetwork(state_dim + 2 * action_dim, action_dim, hidden_dim=self.hidden_dim,
                                             layers=layers).to(self.device)
            self.inner_policy_net = QNetwork(state_dim + 2 * action_dim, action_dim, self.hidden_dim, layers).to(
                self.device)

            self.meta_net = QNetwork(state_dim + 2 * action_dim, action_dim, hidden_dim=self.hidden_dim,
                                     layers=layers).to(self.device)
        ...
        self.meta_lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(self.meta_optimizer, T_max=self.num_task_batches,
                                                                      eta_min=self.meta_min_lr)
    ...
    def inner_update(self, pop_buffer: bool, ret_gradient_ptr: Optional[List[torch.Tensor]] = None,
                     apply_update: bool = True) -> torch.Tensor:
        ...
        # ==target_q_values
        non_final_mask = torch.tensor(tuple(map(lambda s: s is not None, next_states)), dtype=torch.bool).to(
            self.device)
        ...
        grads = torch.autograd.grad(loss, self.inner_policy_net.parameters(), create_graph=self.use_second_order,
                                    allow_unused=True)
        ...
        if apply_update:
            self.inner_policy_net.load_state_dict(
                {name: param - self.inner_lr * grad for ((name, param), grad) in zip(weights.items(), grads)})
        ...
        if ret_gradient_ptr != None:
            ret_gradient_ptr.append(
                grads)  # just append the gradients to the list, we can access the list like a pointer to the next empty element in the buffer
    ...
    def select_action(self, state: torch.Tensor) -> torch.Tensor:
        ...
        if self.useMetaSampling:
            ...
            # ideas: clip so that the values are not too high, or use a different activation function or use weighted softmax
            '''
            weight = torch.sigmoid((self.alpha + self.beta - threshold) / scaling_factor)
            meta_values = self.inner_policy_net(torch.cat([
                torch.tensor([state]).to(self.device),
                torch.tensor(self.alpha, dtype=torch.float32).to(self.device),
                torch.tensor(self.beta, dtype=torch.float32).to(self.device)
            ]))
            ts_values = torch.tensor(
                [np.random.beta(self.alpha[i], self.beta[i]) for i in range(self.action_dim)]
            ).to(self.device) * q_values.detach()
            sampled_values = weight * meta_values + (1 - weight) * ts_values
            '''
            ...
            sampled_values = torch.tensor(
                [np.random.beta(self.alpha[i], self.beta[i]) for i in range(self.action_dim)]).to(
                self.device) * q_values.detach()
    ...
    @overload
    def inner_loop(self, task: RLTask, train: Literal[True] = True, ret_memory_ptr: Optional[MemoryBuffer] = None,
                   ret_gradient_ptr: Optional[List[torch.Tensor]] = None) -> Tuple[float, torch.Tensor]:
        pass

    @overload
    def inner_loop(self, task: RLTask, train: Literal[False] = False, ret_memory_ptr: Optional[MemoryBuffer] = None,
                   ret_gradient_ptr: Optional[List[torch.Tensor]] = None) -> Tuple[
        float, torch.Tensor, torch.FloatTensor]:
        pass

    def inner_loop(self, task: RLTask, train: bool = True, ret_memory_ptr: Optional[MemoryBuffer] = None,
                   ret_gradient_ptr: Optional[List[torch.Tensor]] = None) -> Tuple[float, torch.Tensor] | Tuple[
        float, torch.Tensor, torch.FloatTensor]:
        ...
        for env, init_state in (task.train_sample() if train else task.test_sample()):
            ...
            for s in range(self.num_steps):
                ...
                next_state, reward, terminated, truncated, _ = env.step(
                    action.numpy())  # take the action in the environment
                ...
                self.memory.append((state, [e for e in self.alpha + self.beta], action, reward, next_state,
                                    terminated))  # store the experience
        ...
        return (total_score, torch.stack(env_losses).mean()) if train else (
        score, torch.stack(env_losses).mean(), torch.stack(indiv_losses))
    ...
    def meta_learn(self, task: RLTask | List[RLTask]) -> Tuple[float, float]:
        ...
        for t, __task in tqdm(enumerate(tasks), "Task i from Current Task Batch", total=len(tasks)):
            ...
            task_train_score, task_train_loss = self.inner_loop(__task, train=True,
                                                                ret_gradient_ptr=train_gradients)  # train the inner-net
...
def learn_meta_net(env_name: str = "RandomCartPole", n_tasks: int = 1000, task_batch_size: int = 10, episodes: int = 30,
                   n_steps: int = 500, **kwargs):
    """
    ...
    meta_net: MetaNet = MetaNet(state_dim, action_dim, num_task_batches=n_tasks, num_steps=n_steps,
                                **kwargs)  # total_epochs is task_iterations
    ...
    test_task_set: List[RLTask] = [generate_task(envset.sample(), episodes=episodes, portion_test=0.5) for _ in
                                   range(n_tasks)]
    ...
    wandb.init(project="meta-rl-thoml",
               config=kwargs | {"env_name": env_name, "n_tasks": n_tasks, "episodes": episodes, "n_steps": n_steps})
...
def run_experiment():
    ...
    parser.add_argument("--task_batch_size", type=int, default=10,
                        help="The size of the task batches, only used if use_task_batches is True.")
    parser.add_argument("--update_method", type=str
    default = "reptile", help = "Whether to use second order gradients or not.")
    parser.add_argument("--num_episodes", type=int, default=100,
                        help="The number of episodes to run each environment for.")
    parser.add_argument("--num_steps", type=int, default=500,
                        help="The number of steps to be done in each environment episode.")
    parser.add_argument("--hpt", type=bool, default=False,
                        help="Whether to use hyperparameter tuning or not. All not given, hyperparameters are searched for.")
    parser.add_argument("--config", type=str, default="",
                        help="The json-config file to be used for hyperparameter tuning. See the documentation for the format.")
